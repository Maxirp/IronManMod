...
typedef struct Taudiofile {
#ifdef __cplusplus
	Taudiofile() {};
	Taudiofile(HSTREAM _audio,bool _is3d,	bool _ispaused) : audio(_audio), is3d(_is3d), ispaused(_ispaused) {};
#endif
	HSTREAM audio;
	bool is3d;
	bool ispaused;
...
typedef bool (WINAPI *TBASS_ChannelSet3DAttributes)(
	DWORD handle,
	int mode,
	float min,
	float max,
...
typedef HSTREAM(WINAPI *TBASS_StreamCreateFile)(bool mem, void *file, QWORD offset, QWORD length, DWORD flags);
typedef bool(WINAPI *TBASS_ChannelPlay)(HSTREAM handle, bool restart);
typedef bool(WINAPI *TBASS_ChannelStop)(HSTREAM handle);
typedef bool(WINAPI *TBASS_ChannelPause)(HSTREAM handle);
//typedef void(WINAPI *TBASS_Pause)();
typedef BOOL(WINAPI *TBASS_Init)(
	int device,
	DWORD freq,
	DWORD flags,
	HWND win,
...
typedef DWORD(WINAPI *TBASS_ChannelIsActive)(
	DWORD handle
	);
//typedef void(WINAPI *TBASS_Start)();

typedef void(WINAPI *TBASS_ChannelSetAttribute)(
	DWORD handle,
	int volumevalue2,
	float volume
	);
...
typedef BOOL(WINAPI *TBASS_ChannelSet3DPosition)(
	DWORD handle,
	BASS_3DVECTOR *pos,
...
typedef bool(WINAPI *TBASS_Set3DFactors)(
	float distf,
	float rollf,
	float doppf
	);

typedef bool(WINAPI *TBASS_Set3DPosition)(
	BASS_3DVECTOR *pos,
	BASS_3DVECTOR *vel,
	BASS_3DVECTOR *front,
	BASS_3DVECTOR *top
	);
typedef void(WINAPI *TBASS_Apply3D)();
typedef int(WINAPI *TBASS_ErrorGetCode)();


typedef BOOL(WINAPI *TBASS_GetDeviceInfo)(
	DWORD device,
	BASS_DEVICEINFO *info);

typedef bool(WINAPI *TBASS_GetInfo)(
	BASS_INFO *info);

typedef bool(WINAPI *TBASS_StreamFree)(HSTREAM handle);
...
class AUDIOLIB {
	...
	bool bUseFPAudio;
public:
	AUDIOLIB() : initialized(false), forceDevice(-1), paused(false), bUseFPAudio(false)
	{
		// TODO: give to user an ability to force a sound device to use (ini-file or cmd-line?)

	}
	bool Init();
	TBASS_GetVersion GetVersion;
	TBASS_GetDeviceInfo GetDeviceInfo;
	TBASS_GetInfo GetInfo;
	TBASS_StreamFree StreamFree;
	static void PlayVoiceEvent(int id);
	Taudiofile MenuStream[7];
	HMODULE library;
	//TBASS_Start Start;
	TBASS_Init BassInit;
	TBASS_StreamCreateFile StreamCreateFile;
	TBASS_ChannelPlay ChannelPlay;
	TBASS_ChannelPause ChannelPause;
	TBASS_ChannelStop ChannelStop;
	TBASS_ChannelIsActive ChannelIsActive;
	TBASS_ChannelSet3DPosition ChannelSet3DPosition;
	TBASS_Set3DPosition Set3DPosition;
	TBASS_Set3DFactors Set3DFactors;
	TBASS_ChannelSetAttribute ChannelSetAttribute;
	TBASS_ChannelSet3DAttributes ChannelSet3DAttributes;
	TBASS_Apply3D Apply3D;
	TBASS_ErrorGetCode ErrorGetCode;
	bool isset();
	void Readfromdll();
	static void PlayMenuSFX(int id);
